...
class AuthApiClient(
    ...
    private fun postJson(endpoint: String, body: JSONObject, bearerToken: String?): String {
        ...
        return try {
            ...
            if (connection.responseCode !in 200..299) {
                val errorMessage = runCatching { JSONObject(payloadText).optString("message") }.getOrDefault("")
                val fallback = if (errorMessage.isBlank()) payloadText else errorMessage
                throw IllegalStateException(
                    if (fallback.isBlank()) "Request failed (${connection.responseCode})." else fallback
                )
            }
            payloadText
        } finally {
    ...
    private fun createConnection(url: URL, method: String, bearerToken: String?): HttpURLConnection {
